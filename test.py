...
def parse_args():
    ...
    parser.add_argument("--query_annotation_path", help="Path to the query annotations")
    parser.add_argument("--gallery_data_root", help="Path to the gallery images")
    parser.add_argument("--gallery_annotation_path", help="Path to the query annotations")
...
def rank_gallery_features(query_feature, gallery_features, gallery_ids):
    cosines = [cos_sim(query_feature, gallery_feature) for gallery_feature in gallery_features]
...
if __name__ == "__main__":
    ...
    query_filepaths, query_ids = parse_annotation(args.query_data_root, args.query_annotation_path)
    gallery_filepaths, gallery_ids = parse_annotation(args.gallery_data_root, args.gallery_annotation_path)

    query_features = []
    gallery_features = []
    for query_image_path, gallery_image_path in tqdm(zip(query_filepaths, gallery_filepaths)):
        query_features.append(extractor(query_image_path)[0].cpu().tolist())
        gallery_features.append(extractor(gallery_image_path)[0].cpu().tolist())

    size = len(query_features)
    rank_res = [0] * k
    for feature, query_id in tqdm(zip(query_features, query_ids)):
        ranked_ids = rank_gallery_features(feature, gallery_features, gallery_ids)[:k]
